...
export function RegisterProgressModal({ project, onClose, onSaved }: RegisterProgressModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.55)" }} onClick={onClose}>
      <div className="w-full max-w-lg bg-white rounded-lg shadow-2xl overflow-hidden" onClick={(event) => event.stopPropagation()}>
        <div className="p-5" style={{ backgroundColor: "#000" }}>
          <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 10, fontWeight: 900, textTransform: "uppercase" }}>Registrar avance</p>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between">
              <label style={{ fontSize: 12, fontWeight: 900 }}>Avance global</label>
              <span style={{ fontSize: 22, fontWeight: 950, color }}>{progressPercent}%</span>
            </div>
            <div className="flex items-center gap-3 mt-2">
              ...
              <input type="number" min={0} max={100} step={0.5} value={progressPercent} onChange={(event) => setProgressPercent(Math.max(0, Math.min(100, Number(event.target.value))))} style={{ width: 78, padding: "7px 9px", border: "1.5px solid #000", borderRadius: 6, fontSize: 12, fontWeight: 850 }} />
            </div>
            <div className="mt-2"><ProgressBar value={progressPercent} color={color} /></div>
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 900 }}>Comentario *</label>
            <textarea value={comment} onChange={(event) => { setComment(event.target.value); setError(""); }} rows={4} placeholder="Describe lo que se avanzo, decisiones o bloqueos..." style={{ width: "100%", marginTop: 6, padding: "10px 12px", border: "1.5px solid #000", borderRadius: 6, fontSize: 13, resize: "vertical" }} />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 900 }}>Hitos opcionales</label>
            <textarea value={milestones} onChange={(event) => setMilestones(event.target.value)} rows={2} placeholder="Entrega de prototipo funcional" style={{ width: "100%", marginTop: 6, padding: "10px 12px", border: "1.5px solid #E5E7EB", borderRadius: 6, fontSize: 13, resize: "vertical" }} />
          </div>
        </div>
